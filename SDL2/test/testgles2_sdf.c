...
/*
 * Create shader, load in sources, compile, dump debug as necessary.
 *
 * shader: Pointer to return created shader ID.
 * sources: Passed-in shader sources code.
 * shader_type: Passed to GL, e.g. GL_VERTEX_SHADER.
 */
void
process_shader(GLenum *shader, const char *sources, GLenum shader_type)
{
    ...
    *shader = GL_CHECK(ctx.glCreateShader(shader_type));
    ...
    /* Dump debug info (sources and log) if compilation failed. */
    if (status != GL_TRUE) {
